...
Устойчивость (два холодных прогона ред. 8 → 9 подряд): структура совпадает полностью; извлечено 61 и 58 функций «до»; все выводы второго прогона есть в первом; golden 14/14 в обоих. Этого добились тремя приёмами: функции извлекаются отдельным запросом на каждое подразделение (модель не бросает длинную задачу на полпути); пункт, дословно сохранившийся в новой редакции, не считается потерей; кандидаты на сопоставление берутся и из пункта, куда переехал исходный пункт по выравниванию.

Парсер восстанавливает автонумерацию Word («1.1.», «а)», которых нет в тексте абзаца), строки таблиц становятся пунктами `т1.2`, документ без нумерации получает ссылки на абзацы `абз.5`. Ссылки внутри текста заключения проверяет код (`refcheck.py`): неподтверждённая помечается «⚠ ссылка не подтверждена».
...
|---|---:|---:|---:|
| `gpt-5.4-mini` (извлечение; функции — отдельный запрос на каждое подразделение) | 11 | ~247 000 | ~18 000 |
| `gpt-5.4` (сопоставление, проверки, критик, заключение) | 14–16 | ~61 000–72 000 | ~7 500 |
| `text-embedding-3-small` | 16–18 | ~74 000 | — |

Время: **~1,5–2 мин** холодный прогон, **< 1 с** повторный (всё из кэша `AI_CACHE_DIR`). Точную сумму смотрите в Usage кабинета OpenAI: при таком расходе токенов бюджета хакатона хватает на много прогонов. Для демо заранее прогоните контрольный комплект — тогда на защите результат придёт из кэша мгновенно.
