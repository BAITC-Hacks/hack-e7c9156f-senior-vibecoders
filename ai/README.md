...
## Понятный пример: интернет-магазин

Чтобы проверить агента без знания аудита, есть второй комплект — положение об оргструктуре магазина электроники «ДомТех», ред. 1 → ред. 2 (`samples/shop/`, генерируется `python scripts/make_shop_samples.py`). В ред. 2 заложены изменения, которые агент должен найти:

| Заложено | Ожидаемый вывод |
|---|---|
| Склад и Служба доставки объединены в Департамент логистики, создан Отдел маркетинга | `merged` СКЛ+СД → ДЛ, `created` ОМ |
| Ежемесячную инвентаризацию (ред. 1, п. 4.2.4) больше никто не делает | `function_loss` |
| Жалобы на доставку рассматривают и ДЛ (п. 4.2.6), и СПК (п. 4.4.3) | `duplication` |
| Отдел закупок сам выбирает поставщика (п. 4.3.1) и сам принимает его товар (п. 4.3.3) | `conflict_of_interest` (правило SOD-G1) |

```powershell
python -m ai.cli --before "../samples/shop/before/*.docx" --after "../samples/shop/after/*.docx" --out result_shop.json
python evals/run_evals.py result_shop.json --golden evals/golden_shop.json --strict
```

Результат 2026-09-23: **10/10 golden-фактов**, 21/21 ссылок проверены, ~40 с на холодном кэше.
