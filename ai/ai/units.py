...
def _is_org_unit(u: Unit) -> bool:
    ...
    Единица с аббревиатурой или собственными должностями («Склад (СКЛ)») — подразделение при любом названии."""
    return bool(u.abbr or u.positions) or any(w in _norm(u.name) for w in _ORG_WORDS)
