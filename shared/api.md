...
### Задачи фронтенда (по приоритету — каждая добавляет баллы)

1. **Проверка выводов сотрудником** — закрывает требование ТЗ (разд. 9): «выводы носят рекомендательный характер и требуют проверки ответственным сотрудником».
   - У **каждого** вывода — и в `findings`, и в `rejected_findings` — кнопки **«Подтвердить»** и **«Отклонить»** + поле комментария (для «Отклонить» просить указать причину, но не требовать).
   - Запрос: `PATCH /api/analyses/{id}/findings/{finding_id}`, тело `ReviewRequest`. Ответ — **полный `AnalysisResult`** с уже пересобранным `conclusion_md`: заменяем им всё состояние результата, локально ничего не «допатчиваем».
   - Ответ идёт до ~15 с (ИИ переписывает заключение): на это время — индикатор «Пересобираем заключение…», кнопки этого вывода заблокированы, остальной экран живой.
   - Отображение `review`: нет → «Не проверено»; `accepted` → зелёная плашка «Подтверждено сотрудником» + комментарий + дата (`reviewed_at`); `rejected` → вывод приглушён, плашка «Отклонено сотрудником» + причина. Решение можно изменить повторным PATCH.
   - Счётчик сверху: «Проверено 3 из 8 выводов».
   - `rejected_findings` — отдельный свёрнутый блок **«Отклонено агентом-критиком (N)»** с теми же кнопками: сотрудник может вернуть вывод, подтвердив его (тогда он попадёт в заключение).
   - Ошибки: показываем текст из `error`; 409 — «Анализ ещё не завершён», 404 — «Вывод не найден»; при ошибке состояние не меняем.
2. **Вердикт агента-критика** у каждого вывода (`critic`): `upheld` — «Подтверждено критиком», `uncertain` — жёлтый бейдж «Требует проверки», `refuted` — «Опровергнуто критиком». Раскрывашка: `argument` + `counter_evidence` (каждая — с кнопкой «Источник»).
3. **Панель «Источник»** — у каждого `Evidence`: документ (`doc_name`), «до/после», «п. {clause_id}», полный текст пункта из `result.documents` (или `GET …/clauses/{clause_id}`), цитата подсвечена (поиск без учёта регистра, «ё/е» и лишних пробелов). `verified: false` → пометка «⚠ цитата не подтверждена».
4. **Заключение** — `conclusion_md` рендерим как markdown; фрагменты «⚠ ссылка не подтверждена» выделяем. На экране результата **постоянная плашка** (не только внутри текста): «Выводы носят рекомендательный характер и требуют проверки ответственным сотрудником».
5. **Выводы** — сортировка `high → medium → low`, фильтр по типу. Подписи: `structure_change` — «Изменение структуры», `function_loss` — «Возможная потеря функции», `duplication` — «Дублирование функций», `conflict_of_interest` — «Конфликт интересов».
6. **Redline** (`alignments`) — документы «до/после» бок о бок: `unchanged` без выделения, `modified` — пословный `diff` (`delete` красным зачёркнутым, `insert` зелёным), `added` — зелёный пункт, `removed` — красный, `moved` — пометка «перенесён из п. X». Кнопка «Источник» открывает redline на нужном пункте.
7. **Sankey** (`flows`) — слева единицы «до», справа «после» (подписи из `units`: `abbr` или `name`), `target_unit_id = "lost"` — красный узел «Потеряно». Клик по потоку — список функций (`function_ids` → `function_mappings`).
8. **Прогресс анализа** — опрос `GET /api/analyses/{id}` раз в 1–2 с; подписи шагов: `parsing` «Читаем документы», `alignment` «Сравниваем пункты», `units` «Ищем подразделения», `functions` «Сопоставляем функции», `conflicts` «Проверяем конфликты интересов», `evidence` «Проверяем цитаты», `critic` «Агент-критик перепроверяет выводы», `report` «Пишем заключение». `failed` — показать `error` и кнопку «Повторить».
9. **Загрузка** — ошибки `400/413/415` показываем текстом из `error` рядом с полем загрузки.
10. **История** — `GET /api/analyses`: список прошлых запусков (дата, документы, статус, счётчики) с переходом к результату.
11. **Экспорт** — кнопка «Скачать служебную записку» → `GET …/report.docx`; после проверки выводов записка содержит актуальные отметки сотрудника.
...
- 2026-09-23 — задачи фронтенда: проверка выводов сотрудником (кнопки, PATCH, отображение `review`, блок `rejected_findings`), вердикт критика, панель «Источник», redline, Sankey, прогресс, история, экспорт.
